...
def calib(images, internal_corners):
    ...
    objp = np.zeros((1, internal_corners[0]*internal_corners[1], 3), np.float32)
    ...
    for fname in images:
        ...
        ret, corners = cv2.findChessboardCorners(gray, internal_corners, cv2.CALIB_CB_ADAPTIVE_THRESH+cv2.CALIB_CB_FAST_CHECK+cv2.CALIB_CB_NORMALIZE_IMAGE)
...
def undistort2_img_path(img_path, K, D, DIM,balance=0.0, dim2=None, dim3=None):
    img = cv2.imread(img_path)
    dim1 = img.shape[:2][::-1]  #dim1 is the dimension of input image to un-distort
